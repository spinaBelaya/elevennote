...
class IndexTests(TestCase):
    ...
    def setUp(self):
        self.test_user1 = User.objects.create_user_(
            email="test_user1@example.com",
            password="secret")
        self.test_user2 = User.objects.create_user_(
    ...
    def test_index_url_resolves_index_view(self):
        view = resolve('/notes/') # поиск по всем urls. Совпадение было в общих url где notes included just / which responses to NoteList view 
    ...
    def test_index_view_contains_link_to_details_page(self):
        ...
        index_page_url = reverse('notes:index')
        response = self.client.get(index_page_url)
        for note in response.context['latest_note_list']: #во вьюхах мы добавляем в контекст latest_note_list
            note_detail_url = reverse('notes:update',
                kwargs={'pk': note.pk})
...
class DetailTests(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        ...
        self.assertRedirects(response, f"/accounts/login/?next=/notes/{self.note.pk}/") #(запрос, ожидаемая ссылка)
...
class CreateViewTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user_(
...
class UpdateViewTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user_(email = 'user@example.com', password = 'secret')
        self.note = Note.objects.create(title = "Note title", body = "Note description", owner = self.user)
    ...
    def test_response_contains_notes_list(self):
        ...
        self.assertIn('notes', response.context)
        self.assertQuerysetEqual(response.context['notes'], ['<Note: Note title>'])
    ...
    def only_user_can_update_note(self):
        not_owner = User.objects.create_user_(email = 'not_owner@example.com', password = 'secret')
